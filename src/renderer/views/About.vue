<template>
    <div class="flex flex-col items-center justify-center h-[calc(100vh-9rem)]">
        ...
        <h2 class="mb-0">WinRocket</h2>
        <p class="text-sm text-gray-500">Windows for 🐧 penguins</p>
        <p class="text-gray-400 !mt-4">WinRocket Beta v{{ appVer }} {{ isDev ? "Dev" : "Prod" }}</p>
        <div></div>
        <p class="text-gray-400 !mt-16">
            WinRocket is
            <a @click="openAnchorLink" href="https://github.com/mishastarrrr-ctrl/WinRocket">open-source software</a>
        </p>
        <p class="text-gray-400 !mt-0">Copyright (c) 2025 TibixDev</p>
        <p class="text-sm text-gray-500 text-center !mt-16 w-[40vw]">
            WinRocket is not affiliated with Microsoft Corporation, nor is it endorsed or sponsored by them. "Windows" is
